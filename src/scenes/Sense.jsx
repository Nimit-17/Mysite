...
export default function Sense() {
  ...
  useLayoutEffect(() => {
    ...
    const ctx = gsap.context((self) => {
      ...
      gsap.set(q(".forget"), { yPercent: 160, rotate: 6, autoAlpha: 0 });
      ...
      tl.to(q(".photo-pool"), { yPercent: 0, rotate: -2, autoAlpha: 1, duration: 0.8, ease: "power3.out" }, 0)
        ...
        // ...and a beat later, the follow-up card slaps in under it
        .to(q(".forget"), { yPercent: 0, rotate: 1.5, autoAlpha: 1, duration: 0.55, ease: "back.out(2)" }, 3.9)
        .to({}, { duration: 0.5 });
    }, root);
    return () => ctx.revert();
  }, []);
        ...
        <div className="relative mx-auto grid w-full max-w-6xl items-center gap-5 md:grid-cols-[1.1fr_0.9fr] md:gap-14">
          ...
          <div className="order-2 min-w-0 max-w-xl">
            ...
            <div className="beat-stack mt-4 md:mt-6">
              ...
              {/* beat 2 — the memory gag: two cards, the second undercuts the first */}
              <div className="self-start [grid-area:1/1]">
                <div className="note sticker t-scene w-fit max-w-full text-[clamp(1.1rem,0.85rem+1.3vw,1.9rem)]">
                  I make a list of things to remember
                </div>
                <div
                  className="forget sticker t-scene mt-4 w-fit max-w-full bg-ink text-paper text-[clamp(1.1rem,0.85rem+1.3vw,1.9rem)] md:mt-5"
                  style={{ boxShadow: "4px 5px 0 var(--color-rust)" }}
                >
